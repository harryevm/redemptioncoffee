  ...
  $(".quick-view").click(function () {
    ...
    jQuery.getJSON('/products/' + product_handle + '.js', function (product) {
      ...
      $(images).each(function (i, image) {
        ...
        if (i == imageCount - 1) {
          ...
          $('.qv-product-thumb').append(image_embed);

 

          $('.qv-product-thumb').slick({
      ...
      $(options).each(function (i, option) {
        ...
        var selectClass = '.option.' + opt.replace(/[^A-Z0-9]/ig, '_').toLowerCase();

        $('.qv-product-options').append('<div class="option-selection-' + opt.replace(/[^A-Z0-9]/ig, '_').toLowerCase() + '"><span class="option">' + opt + '</span><select class="option-' + i + ' option ' + opt.replace(/[^A-Z0-9]/ig, '_').toLowerCase() + '"></select></div>');

        $(option.values).each(function (i, value) {

          $(selectClass).append('<option value="' + value + '">' + value + '</option>');
    ...
    $.fancybox({
      ...
      'beforeLoad': function () {
        ...
        $(document).on("click", ".increment-count", function () {
        	var qtVal = $('.qv-quantity').val();
         	qtVal++;
           $('.qv-quantity').val(qtVal);
        });
        $(document).on("click", ".decrement-count", function () {
        	var qtVal = $('.qv-quantity').val();
          if(qtVal>0){
         	qtVal--;
          }
      ...
      'afterShow': function () {
        ...
        $('#quick-view').hide().html(content).css('opacity', '1').fadeIn(function () {
          ...
          $('.qv-product-thumb').addClass('loaded');
...
$(window).resize(function () {
  ...
  if ($('#quick-view').is(':visible')) {
    ...
    $('.qv-product-thumb').slick('setPosition');
